...
<template>
	<v-card class="mx-auto my-auto" width="350" elevation="8" rounded>
		...
		<v-img v-if="typeof venue.src === 'string'" class="card-bg" :src="venue.src" height="250" cover>
		...
		<v-card-item>
			<v-card-title v-text="venue.name" />

			<v-card-subtitle>
				<span class="me-1">{{ venue.location }}</span>
			</v-card-subtitle>

			<v-chip-group v-if="venue.events.length > 0" mandatory>
				<v-menu v-for="(event, i) in venue.events" :key="i" v-model="menus.events[i]" location="top start" origin="top start" transition="scale-transition">
		...
		<v-card-text class="m-3 p-0 clamp" v-text="venue.description" />
